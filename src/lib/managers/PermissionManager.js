...
class PermissionManager {
  ...
  match(user, host) {
    const sql =
      'select distinct public_key from accounts a, public_keys k, permissions p where a.status = 1 and a.id = k.account_id and and a.id = p.account_id and ? like p.host and ? like p.user';
